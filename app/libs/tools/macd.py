...
from app.libs.utils.feature_utils import normalize_signals
from app.libs.utils.generic_utils import date_extractor_list
from app.libs.utils.constants import TREND_COLORS, STANDARD_COLORS

from .moving_average import exponential_moving_avg_generic
...
def mov_avg_convergence_divergence(ticker: str, **kwargs) -> dict:
    ...
    if is_already_valid_data(ticker, position, 'macd'):
        return position['macd']

    fund = position['ochl']

    macd = generate_macd_signal(fund)

    macd = get_macd_statistics(macd)
...
def generate_macd_signal(fund: pd.DataFrame, **kwargs) -> dict:
    ...
    emaTw = exponential_moving_avg_generic(fund, interval=12)
    emaTs = exponential_moving_avg_generic(fund, interval=26)
    macd_val = []
    ...
    macd_sig = exponential_moving_avg_generic(
        macd_val, interval=9, data_type='list')
...
def macd_metrics(position: pd.DataFrame, macd: dict, **kwargs) -> dict:
    ...
    metrics = exponential_moving_avg_generic(m_bar, 7, data_type='list')
...
def get_macd_features(macd: dict, position: pd.DataFrame) -> list:
    ...
    if isinstance(position, dict):
        ...
        position = position.set_index('dates')
...
def get_macd_value(macd: list, value_type='current', index=-1) -> float:
    ...
    if value_type == 'current':
        ...
        macd_ema = exponential_moving_avg_generic(
            macd, interval=3, data_type='list')
        ...
        if state == 1:
            ...
            while (i > 0) and (state == 1):
                if macd[i] > 0.0:
                    ...
                    state = 0
            m_ema = exponential_moving_avg_generic(
