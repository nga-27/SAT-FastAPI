...
from app.libs.utils.db_utils import is_already_valid_data, patch_db, download_data
from app.libs.utils.classes import Ticker

from app.libs.utils.generic_utils import date_extractor, INDEXES
from app.libs.utils.feature_utils import normalize_signals
from app.libs.utils.math_functions import lower_low, higher_high, bull_bear_th
from .moving_average import exponential_moving_avg_generic


def ultimate_oscillator(ticker: str, **kwargs) -> dict:
    """Ultimate Oscillator
    ...
    """
    config = kwargs.get('config', [7, 14, 28])
    ticker = Ticker(ticker=ticker)

    position = download_data(ticker)
    if is_already_valid_data(ticker, position, 'ultimate_oscillator', config=config):
        return position['ultimate_oscillator']
    ...
    ultimate = ultimate_osc_metrics(position, ultimate)
    ...
    payload = {'signal': ultimate}
    patch_db(ticker, 'ultimate_oscillator', payload)
...
def ultimate_osc_metrics(position: pd.DataFrame, ultimate: dict, **kwargs) -> dict:
    ...
    metrics = exponential_moving_avg_generic(state2, 7, data_type='list')
    if p_bar is not None:
        p_bar.uptick(increment=0.1)
